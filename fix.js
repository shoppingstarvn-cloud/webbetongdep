/* fix.js v11 - ConcretePro Global Button, Link & Module Fixer
   Changes v11:
   - BUG#6 FIX: Download Brochure regex .{0,5} → .{0,15} to match "Download Technical Brochure"
*/
(function () {
'use strict';
...
function setupProductsPage() {
  ...
  document.querySelectorAll('button').forEach(function(btn) {
    ...
    if ((icon === 'grid_view' || icon === 'view_list') && !btn.onclick) {
      btn.onclick = function() {
        ...
        var grid = cards.length ? cards[0].parentElement : null;
        if (!grid) return;
        if (icon === 'view_list') { grid.style.gridTemplateColumns = '1fr'; }
  ...
  document.querySelectorAll('button').forEach(function(btn) {
    var t = btn.textContent.trim();
    if (t.includes('chevron_left') && !btn.onclick) { btn.onclick = function() { showProductPage(prodCurrentPage - 1); }; btn.style.cursor = 'pointer'; }
    if (t.includes('chevron_right') && !btn.onclick) { btn.onclick = function() { showProductPage(prodCurrentPage + 1); }; btn.style.cursor = 'pointer'; }
  });
}
...
function setupSearch() {
  ...
  if (!sInput || sI9nput.oninput) return;
